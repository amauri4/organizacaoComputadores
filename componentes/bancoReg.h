...
SC_MODULE(RegisterFile)
{
    ...
    void read()
    {
        // Leitura combinatória - sempre atualizada
        read_data1.write(registers[read_reg1.read()]);
        read_data2.write(registers[read_reg2.read()]);

        // Debug
        cout << "REGISTER FILE [READ] @ " << sc_time_stamp() << ":\n"
             << "  $" << read_reg1.read() << " = 0x" << hex << registers[read_reg1.read()] << "\n"
             << "  $" << read_reg2.read() << " = 0x" << hex << registers[read_reg2.read()] << endl;
        if (clk.posedge())
        {
            // Atualiza saídas imediatamente se estiver lendo o mesmo registrador
            if (read_reg1.read() == write_reg.read())
            {
                read_data1.write(write_data.read());
            }
            if (read_reg2.read() == write_reg.read())
            {
    ...
    void write()
    {
        // Escrita sincronizada na borda positiva
        if (clk.posedge())
        {
            if (reg_write.read() && write_reg.read() != 0)
            {
                registers[write_reg.read()] = write_data.read();
                cout << "REG WRITE @ " << sc_time_stamp()
                     << " | $" << write_reg.read()
    ...
    SC_CTOR(RegisterFile)
    {
        ...
        // Configura métodos sensíveis
        SC_METHOD(read);
        sensitive << read_reg1 << read_reg2; // Leitura combinatória
        dont_initialize();

        SC_METHOD(write);
        sensitive << clk.pos(); // Escrita na borda positiva
        dont_initialize();
    }
};